...
img_croped = warpCropimage(image, rect)
...
# Draw boxes and show the images
#pprint(rows)
drawBoxes(rows)
printResults(rows)
img_show = cv2.resize(img_croped, (0,0), fx=0.2, fy=0.2)
#verts_show = cv2.resize(verts, (0,0), fx=0.2, fy=0.2)
cv2.imshow("cropped",img_show)
#cv2.imshow("masked",verts_show)
#cv2.imshow("images", np.hstack([img_show, verts_show]))
cv2.waitKey(0)
